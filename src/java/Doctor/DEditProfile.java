...
public class DEditProfile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("update   doctorreg set dspec='"+spec+"', dname='" + name + "',address='" + Address + "',dcity='" + city + "',dphone='" + Contatct + "',pincode='" + pincode + "' where did='" + uid + "'");
            int i = ps.executeUpdate();
            if (i == 1) {
                response.sendRedirect("Doctor/EditProfile.jsp?res=Success");
            } else {
                response.sendRedirect("Doctor/EditProfile.jsp?res=Failed");
            }
